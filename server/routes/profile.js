...
router.post("/technology", auth, async (req, res) => {
  try {
    const { discussionMatter, specificOption, collaborativeOptions, projectMode } = req.body.onboarding.details;

    // only users have technologies
    if (req.user.role !== "user") {
      return res.status(403).json({ message: "Only users can add technologies" });
    }
    ...
    if (discussionMatter === "collaborative" && projectMode === "collaborative") {
      user.onboarding.details.discussionMatter = "collaborative";
      user.onboarding.details.projectMode = "collaborative";
      user.onboarding.details.collaborativeOptions = collaborativeOptions;
      await user.save();
      return res.json(user);
    }

    // Handle technology transfer
    const categoryMap = {
    ...
    if (!category && projectMode === "transfer") {
      return res.status(400).json({ message: `Invalid discussionMatter: ${discussionMatter}` });
    }

    if (projectMode === "transfer") {
      user.onboarding.technologies.push({
        item: specificOption,
        category,
      });
      user.onboarding.details.discussionMatter = discussionMatter;
      user.onboarding.details.specificOption = specificOption;
      user.onboarding.details.projectMode = "transfer";
      await user.save();
      return res.json(user);
    }
