...
import TechTransferFlow from "../models/TechTransferFlow.js"; // Import the TechTransferFlow model
...
router.get("/tt-coordinator-actions", auth, adminOnly, async (req, res) => {
  try {
    // Example: get last 50 actions from TechTransferFlow or ActionLog
    const actions = await TechTransferFlow.find({})
      .sort({ updatedAt: -1 })
      .limit(50)
      .populate("user", "name email")
      .populate("coordinator", "name email"); // adjust as per your schema
    // Flatten steps for display
    const flatActions = actions.flatMap(flow =>
      flow.steps.map(step => ({
        _id: flow._id + "-" + step.name,
