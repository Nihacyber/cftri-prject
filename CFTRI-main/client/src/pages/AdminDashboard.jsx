...
const AdminDashboard = () => {
  ...
  const [ttActions, setTTActions] = useState([]);
  const [isLoadingTTActions, setIsLoadingTTActions] = useState(false);

  const [isButtonDisabled, setIsButtonDisabled] = useState(false);
  const [disabledAssignBtnUserId, setDisabledAssignBtnUserId] = useState([]);
                      ...
                      >
                        ...
                        {expandedUsers[user._id] && (
                          <div className="p-4 border-t">
                            {/* Staff Assigned */}
                            {/* Assign Coordinator if not assigned */}
                            {!user.onboarding?.details?.coordinator ? (
                              <>
                                ...
                                {assignForm.show && assignForm.userId === user._id && (
                              ...
                              <div className="mb-2">
                                ...
                                <p className="text-gray-800">
                                  ...
                                  "object"
                                    ...
                                    : user.onboarding.details.coordinator}
                                  {user.onboarding.details.coordinatorAssignedAt && (
                              ...
                              {loadingUserLogs[user._id] ? (
                                ...
                                userLogs[user._id].length > 0 ? (
                                  ...
                                  <p className="text-sm text-gray-500">
                                    No actions logged for this client.
                                  </p>
                                )
                              ) : (
                                <p className="text-sm text-gray-500">
                            ...
                            <button
                              ...
                              onClick={() =>
                                setExpandedUsers((prev) => ({
                                  ...prev,
                                  [user._id]: prev[user._id] === "full" ? true : "full",
                                }))
                              }
                            >
                              {expandedUsers[user._id] === "full"
                                ? "Hide Details"
                                : "Show More Details"}
                            </button>

                            {/* Show the rest of the info only if expandedUsers[user._id] === "full" */}
                            {expandedUsers[user._id] === "full" && (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                                ...
                                {/* Collaborative Options and New Technologies Info side by side */}
          ...
          {activeTab === 8 && (
            <div>
              ...
              {loadingProcessed ? (
                <div className="text-center py-8">
                  <Loader2 className="h-8 w-8 animate-spin text-gray-400 mx-auto" />
                  <p className="text-gray-500 mt-2">Loading completed processes…</p>
                </div>
              ) : processedUsers.length === 0 ? (
                <p className="text-gray-600">No completed processes.</p>
              ) : (
                <div className="overflow-x-auto">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {processedUsers.map((u) => (
                        <tr key={u._id}>
                          ...
                          <td className="px-4 py-2">
                            {u.onboarding.details.ttCoordinator ? (
                              <button
                              ...
                              <button
                                className={`bg-purple-600 text-white px-3 py-1 rounded ${
                                  disabledAssignBtnUserId === u._id
                                    ? "opacity-50 cursor-not-allowed"
                                    : "hover:bg-purple-700"
                                }`}
                                disabled={disabledAssignBtnUserId === u._id}
                                onClick={() => {
                                  setDisabledAssignBtnUserId(u._id);
                                  setUserToAssign(u);
                            ...
                            {showDeanModal && userToAssign && userToAssign._id === u._id && (
                              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                                <div className="bg-white p-6 rounded-lg w-full max-w-md shadow-lg">
                                  ...
                                  <p className="mb-2">
                                    <strong>Client:</strong> {userToAssign.name} (
                                    {userToAssign.email})
                                  </p>
                                  <label className="block mb-2 font-medium">
                                    Select Dean:
                                  ...
                                  <div className="flex justify-end space-x-2">
                                    <button
                                      onClick={() => {
                                        ...
                                        setDisabledAssignBtnUserId(null); // Re-enable if cancelled
                                      }}
                                    ...
                                    <button
                                      onClick={async () => {
                                        ...
                                        setShowDeanModal(false);
                                        // Do NOT remove from disabledAssignBtnUserId, so button stays disabled
